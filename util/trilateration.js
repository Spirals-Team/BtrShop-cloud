 ...
 */
const Beacon = function(lat, lon, dist) {
  this.lat  = lat;
  this.lon  = lon;
  this.dist = dist;
 ...
 */
const trilaterate = function(beacons) {
  ...
  const x =  (math.pow(beacons[0].dist, 2)
    - math.pow(beacons[1].dist, 2)
    + math.pow(d, 2)) / (2 * d);
  const y = ((math.pow(beacons[0].dist, 2)
    - math.pow(beacons[2].dist, 2)
    + math.pow(i, 2)
    + math.pow(j, 2)) / (2 * j)) - ((i / j) * x);
